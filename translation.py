...
# def load_file(hdf5_path, tgt_add_bos=True):
#     en = []
#     fr = []
...
#
#         english_sentences = f['english'][:]
#         french_sentences = f['french'][:]
#
#         for en_sentence, fr_sentence in zip(english_sentences, french_sentences):
#
#             en.append(["BOS"] + nltk.word_tokenize(en_sentence.lower()) + ["EOS"])
#
#
#             fr_doc = nlp(fr_sentence)
#             fr_tokens = [token.text for token in fr_doc]
#             if tgt_add_bos:
#                 fr.append(["BOS"] + fr_tokens + ["EOS"])
#             else:
#                 fr.append(fr_tokens)
#
#     return en, fr
...
def load_file(path, tgt_add_bos=True):
    ...
    with open(path, 'rb') as f:
        for line in f.readlines():
            ...
            if tgt_add_bos:
                fr.append(["BOS"] + line[1].split() + ["EOS"])
            else:
                fr.append(line[1].split())
...
def tokenize2num(en_sentences, fr_sentences, en_word2idx, fr_word2idx, sort_reverse=True):
    length = len(en_sentences)

    out_en_sents = [[en_word2idx.get(word, 1) for word in sen] for sen in en_sentences]
    out_fr_sents = [[fr_word2idx.get(word, 1) for word in sen] for sen in fr_sentences]
    ...
    if sort_reverse:
        ...
        out_fr_sents = [out_fr_sents[idx] for idx in sorted_index]

    return out_en_sents, out_fr_sents
...
class DataProcessor(object):
    def __init__(self,args):
        cached_en_tokenizer = os.path.join(DATA_DIR,"\cached_{}".format("en_tokenizer"))
        cached_fr_tokenizer = os.path.join(DATA_DIR, "\cached_{}".format("fr_tokenizer"))

        if not os.path.exists(cached_en_tokenizer) or not os.path.exists(cached_fr_tokenizer):
            en_sents, fr_sents = load_file(DATA_DIR + "train.txt")
            en_word2idx, en_id2word, en_vocab_size = build_tokenizer(en_sents,args)
            fr_word2idx, fr_id2word, fr_vocab_size = build_tokenizer(fr_sents, args)

            torch.save([en_word2idx, en_id2word, en_vocab_size],cached_en_tokenizer)
            torch.save([fr_word2idx, fr_id2word, fr_vocab_size],cached_fr_tokenizer)
        else:
            en_word2idx, en_id2word, en_vocab_size = torch.load(cached_en_tokenizer)
            fr_word2idx, fr_id2word, fr_vocab_size = torch.load(cached_fr_tokenizer)

        self.en_tokenizer = Tokenizer(en_word2idx, en_id2word, en_vocab_size)
        self.fr_tokenizer = Tokenizer(fr_word2idx, fr_id2word, fr_vocab_size)

    def get_train_examples(self,args):
        return self._create_examples(os.path.join(DATA_DIR, "data/train.txt"), "train", args)


    def get_dev_examples(self,args):
        return self._create_examples(os.path.join(DATA_DIR,"dev.txt"),"dev",args)

    def _create_examples(self,path,set_type,args):
        en_sents, fr_sents = load_file(path)
        out_en_sents, out_fr_sents = tokenize2num(en_sents, fr_sents,
                                                  self.en_tokenizer.word2idx, self.fr_tokenizer.word2idx)
        ...
        for minibatch in minibatches:
            ...
            mb_fr_sentences = [out_fr_sents[i] for i in minibatch]

            mb_x,mb_x_len = prepare_data(mb_en_sentences)
            mb_y,mb_y_len = prepare_data(mb_fr_sentences)
...
def test(args,model,processor):
    model.eval()
    en_sents, fr_sents = load_file(args.data_dir+'test.txt',tgt_add_bos=False)
    en_sents, _ = tokenize2num(en_sents, fr_sents,
                               processor.en_tokenizer.word2idx,
                               processor.fr_tokenizer.word2idx,
                               sort_reverse=False)
    ...
    with torch.no_grad():
        for idx, en_sent in enumerate(test_iteration):
            ...
            bos = torch.Tensor([[processor.fr_tokenizer.word2idx['BOS']]]).long().to(args.device)
            completed_hypotheses = model.beam_search(mb_x, mb_x_len,
                                                     bos, processor.fr_tokenizer.word2idx['EOS'],
                                                     topk=args.beam_size,
                                                     max_length=args.max_beam_search_length)
            top_hypotheses.append([processor.fr_tokenizer.id2word[id] for id in completed_hypotheses[0].value])

    bleu_score = corpus_bleu([[ref] for ref in fr_sents],
                             top_hypotheses)
...
def main():
    ...
    encoder = EncoderBiRNN(processor.en_tokenizer.vocab_size, args.enc_hidden_size)

    decoder = DecoderWithAttention(
        hidden_size=args.dec_hidden_size,
        output_size=processor.fr_tokenizer.vocab_size,
        context_size=args.enc_hidden_size * 2,  # 假设使用双向编码器
        maxout_pool_size=2,  # 示例值
        maxout_layers=1  # 示例值
    )
    ...
    if args.do_translate:
        ...
        model.to(device)
        while True:
            title = input("input la phrase anglaise :\n")
            ...
            bos = torch.Tensor([[processor.fr_tokenizer.word2idx['BOS']]]).long().to(device)
            ...
            for hypothes in completed_hypotheses:
                    result = "".join([processor.fr_tokenizer.id2word[id] for id in hypothes.value])
                    score = hypothes.score
                    print("Le résultat traduit en français est:{},score:{}".format(result,score))
